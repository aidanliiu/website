...
(() => {
  ...
  const info = {
    art4hearts: {
      ...
      tag: 'Leadership / Nonprofit',
      title: 'Art4Hearts International',
      body: `I'm Chapter Director of the Saratoga High chapter of Art4Hearts International — a 501(c)(3) with 5,500+ volunteers worldwide and 22K Instagram followers. They run art workshops for people in crisis: kids in hospitals, families in shelters, communities dealing with trauma. I handle chapter operations, manage outreach, and help direct our programs. Motto is "creating smiles through art, one heart at a time" — sounds cheesy until you've actually seen it work.`,
    ...
    books: {
      ...
      tag: 'Reading list',
      title: 'Books',
      body: `I read mostly at night, usually after practice. Literary fiction, war lit,
        memoir — books where something real is at stake. These are the ones I keep
      ...
      links: [
        { text: 'Browse the full shelf →', href: 'books.html', internal: true },
      ],
    },
    food: {
      accent: '#f1530a',
      ...
      body: `I photograph everything I eat — not for content, just because I like
        being able to go back and find the exact thing I ordered. The album is
        at 250+ photos now. My usual move is the Cupertino plaza: Marufuku
        Ramen, Harumi Sushi, Yogurtland after. Hard to go wrong. Here's the
        full list:`,
      links: [
    ...
    lift: {
      ...
      tag: 'Fitness',
      title: 'Weightlifting',
      body: `Basketball got me into the weight room, but lifting has become its own
        thing. I train PPL on top of team practices — it started as extra work
        and turned into something I genuinely look forward to. And yes, I outlift
        my brother on every machine. Here's what I actually run:`,
      links: [
        { text: 'My PPL routine ↗', href: 'https://docs.google.com/document/d/1KuQI_JTqal5-SGHNBOoL_gmKqtxKIWv6_YlCrmuROzY/edit?tab=t.0' },
      ],
    },
    write: {
      accent: '#6f2ff0',
      ...
      tag: 'Writing',
      title: 'Writing',
      body: `Writing is one of the few things where I'll reread what I made and
        actually feel good about it. These are essays I kept working on past the
        due date — the kind where the topic made me think past the assignment.
    ...
    wordle: {
      ...
      body: `First thing I do every morning — before I check anything else — is
        Wordle and Connections. I've done both every single day for over a year
        now. I don't know exactly when it stopped being a fun habit and started
        being a thing I'd reschedule plans around, but here we are.`,
      links: [
  ...
  const open = (key) => {
    ...
    const linksHtml = links.length
      ? `<div class="info-modal-links">${links
          .map((l, i) => `<a class="info-modal-link${i > 0 ? ' info-modal-link--secondary' : ''}" href="${l.href}"${l.internal ? '' : ' target="_blank" rel="noopener"'}>${l.text}</a>`)
          .join(' ')}</div>`
      : '';
...
(() => {
  ...
  buttons.forEach((btn) => {
    ...
    btn.addEventListener('mousemove', (e) => {
      ...
      btn.style.setProperty('--lx', `${(relX * 2).toFixed(1)}px`);
      btn.style.setProperty('--ly', `${(relY * 2).toFixed(1)}px`);
    });
...
(() => {
  ...
  // Wire up award card click/keyboard — tilt removed, CSS handles hover shadow
